...
const PoolTransactions = ({ address } : PoolTransactions): JSX.Element => {
  ...
  const { data } = useSubscription<PoolTransactionCount, BasicTransactionVar>(
  ...
  const transactionView = !loadingTransactions && transactionData
    ? transactionData.verified_pool_event
      .map(({
        ...
        return (
          <tr key={id}>
            <td className="fs-5"><a href={`${innitialNetwork.reefscanFrontendUrl}/extrinsic/${hash}`}>{description(transactionType, amount_1, symbol1, symbol2)}</a></td>
